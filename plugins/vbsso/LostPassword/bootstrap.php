...
Event::forge('Foolz\Plugin\Plugin::execute.vbsso/LostPassword')->setCall(function ($result) {
    /**
    ...
    if (vbsso_get_platform_config_property(SHAREDAPI_PLATFORM_WORDPRESS, VBSSO_CONFIG_PROPERTY_OVERRIDE_LINKS, TRUE)
        && get_site_option(VBSSO_NAMED_EVENT_FIELD_LOSTPASSWORD_URL, '') != '') {
        add_filter('lostpassword_url', 'vbsso_lostpassword_url_hook', 11);
    }
    ...
    function vbsso_lostpassword_url_hook() {
        return sharedapi_url_add_destination(get_site_option(VBSSO_NAMED_EVENT_FIELD_LOSTPASSWORD_URL, ''), 'server',
