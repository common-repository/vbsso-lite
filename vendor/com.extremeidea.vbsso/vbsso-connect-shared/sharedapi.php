...
if (!defined('SHAREDAPI')) {
    ...
    // Openssl constants
    define ('SHAREDAPI_OPENSSL_CIPHER', "AES-128-CBC");
    ...
    function sharedapi_encrypt($key, $data) {
        ...
        $ivLength = openssl_cipher_iv_length(SHAREDAPI_OPENSSL_CIPHER);
        $iVector = openssl_random_pseudo_bytes($ivLength);
        $cipherTextRaw = openssl_encrypt($data, SHAREDAPI_OPENSSL_CIPHER, $key, OPENSSL_RAW_DATA, $iVector);
        $hmac = hash_hmac('sha256', $cipherTextRaw, $key, true);

        return trim(base64_encode($iVector . $hmac . $cipherTextRaw));
    }
    ...
    function sharedapi_decrypt($key, $data) {
        ...
        $original_plaintext = openssl_decrypt($cipherTextRaw, SHAREDAPI_OPENSSL_CIPHER, $key, OPENSSL_RAW_DATA, $iv);
        $calcMac = hash_hmac('sha256', $cipherTextRaw, $key, true);

        return hash_equals($hmac, $calcMac) ? trim($original_plaintext) : null;
    }
